...
|---|---|---|
| H1 | `TradeRecord` financial fields have no bounds | `Field(gt=0)`, `Field(ge=0)`, `Field(lt=1)` added to all 18 numeric fields |
| M1 | `WsLogEvent` model not used in hot path | `WsLogHandler.emit()` now uses `WsLogEvent.model_dump()` |
| M2 | `stop` command missing from TypeScript contract | `WsStopCommand` added to `shared/types/api.ts` `WsCommand` union |
| M4 | `ttl_seconds` constant defined in three places | `TICKET_TTL_SECONDS` exported from `tickets.py`; `ws_ticket.py` uses it |

### New model added

`BotStatusResponse` added to `schemas.py`:
...
class BotStatusResponse(BaseModel):
    ...
    registered: bool
    running: bool
    halted: bool  # True when circuit breaker has tripped
```

Used by `GET /api/v1/clients/{id}/bots/{botid}/status`.
