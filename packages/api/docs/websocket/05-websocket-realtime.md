...
|---|---|---|
| H1 | `WebSocketManager` module-level singleton | Moved to `app.state.ws_manager` via `_lifespan`; `websocket.py` reads from `app.state` |
| H2 | WS commands lack ownership verification | `botid not in bot_manager.get_botids(client_id)` guard added to `run`/`stop`/`remove` |
| M1 | O(N) log handler overhead | `WsFanOutHandler` attached once at startup — formats each record once, fans out to all queues |
| M2 | Background tasks accumulate in `_tasks` | Noted; low priority at current scale |
| M4 | No protocol version field | Noted; deferred |
| L4 | `stop` command missing from TypeScript | `WsStopCommand` added to `shared/types/api.ts` |

### WsFanOutHandler

`WsFanOutHandler` replaces the per-client `WsLogHandler` pattern:

```
Bot log record
  → WsFanOutHandler.emit() [attached to logging.root at startup]
    → format record ONCE
    → iterate all active queues → put_nowait(event)
```

The per-client `WsLogHandler` and `_attach_log_handler`/`_detach_log_handler` methods are retained for backward compatibility with tests that attach handlers directly, but are no longer used in the production path.

### Logger name fix (SEC-003)

`BotManager` now receives `logging.getLogger("sonarft.api_bridge")`. The `sonarft.*` prefix passes the `_is_bot_record` filter, restoring real-time log streaming to WebSocket clients. Previously all bot records were attributed to `src.services.bot_service` and silently blocked.
