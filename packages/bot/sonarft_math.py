...
# Fee rounding strategy: ROUND_HALF_EVEN (banker's rounding) eliminates systematic bias.
# Set SONARFT_FEE_ROUNDING=HALF_UP to use traditional rounding.
_FEE_ROUNDING = ROUND_HALF_EVEN if os.environ.get('SONARFT_FEE_ROUNDING') != 'HALF_UP' else ROUND_HALF_UP
...
class SonarftMath:
    ...
    def calculate_trade(self, buy_price, sell_price, buy_price_list, sell_price_list, target_amount, base, quote):
        ...
        def d_fee(value, precision):
            """Quantize fee values using configurable rounding (HALF_EVEN by default)."""
            fmt = Decimal(10) ** -precision
            return Decimal(str(value)).quantize(fmt, rounding=_FEE_ROUNDING)
        ...
        sell_fee_d = d_fee(sell_price_d * target_amount_sell_d * Decimal(str(sell_fee_rate)), sell_rules['fee_precision'])
