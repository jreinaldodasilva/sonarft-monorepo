...
| # | Issue | Effort | Priority | Acceptance Criteria |
|---|---|---|---|---|
| ~~LT1~~ | ~~Migrate API calls to canonical paths~~ | ~~1–2h~~ | ~~Low (deadline: Jan 2026)~~ | ✅ **Done** — All 7 client-scoped endpoints migrated to `/clients/{clientId}/...` in `api.ts`; `getOrders`/`getTrades` `clientId` made required; `helpers.ts` updated; MSW handlers updated; `api.test.ts`, `helpers.test.ts`, and `workflows.test.tsx` path assertions updated. 141/141 passing. |
| ~~LT2~~ | ~~Add 401 detection in `api.ts`~~ | ~~1h~~ | ~~Low~~ | ✅ **Done** — `setUnauthorizedHandler` registration function added to `api.ts`; `fetchWithTimeout` invokes it on 401 responses. `AuthProvider` registers the handler via `useEffect`, sets `sessionExpired: true` and calls `handleLogout`. `sessionExpired` added to `AuthContextValue`. `Crypto` page shows an amber banner when `sessionExpired` is true. `handleLogin` resets `sessionExpired`. 3 new tests. 144/144 passing. |
| ~~LT3~~ | ~~Wire `PrivateRoute` into routing or remove~~ | ~~30min~~ | ~~Low~~ | ✅ **Done** — `PrivateRoute` wired into `App.tsx`; extracted `AppRoutes` component to access `AuthContext` inside `Router`. `Crypto` page now only handles the mid-session 401 expiry case (shows session-expired banner); unauthenticated redirect handled by `PrivateRoute`. |
| ~~LT4~~ | ~~Fix modal focus trap~~ | ~~2h~~ | ~~Low~~ | ✅ **Done** — `useFocusTrap` hook created (`Bots/useFocusTrap.ts`): moves focus to first focusable element on open, traps Tab/Shift+Tab within the modal, closes on Escape, restores focus on close. Applied to both live-trading and remove-bot modals via `ref` + `useFocusTrap`. `KeyboardEvent` added to ESLint globals. |
| ~~LT5~~ | ~~Fix `<h1>` in Footer → `<p>`; add sr-only `<h1>` to Crypto~~ | ~~30min~~ | ~~Low~~ | ✅ **Done** — `Footer.tsx`: `<h1>` → `<p>`; `footer.css`: `.footer h1` → `.footer p`. `Crypto.tsx`: `<h1 className="sr-only">SonarFT Trading Dashboard</h1>` added as first child of `<section>`. |
...
| ~~LT7~~ | ~~Add client-side ping timeout~~ | ~~1–2h~~ | ~~Low~~ | ✅ **Done** — `useWebSocket` gains a `setInterval` watchdog (checks every 15s); closes the socket if no message received in 60s (2× server ping interval). Uses `addEventListener("message")` to reset the timer without conflicting with `useBots` `onmessage`. 2 new tests (closes on timeout, does not close when messages arrive). Tests: 144 → 146 passing. |
| ~~LT8~~ | ~~Add empty state messages to history tables~~ | ~~1h~~ | ~~Low~~ | ✅ **Done** — `TradeHistoryTable` shows a "No records yet" row (full-width, italic, muted) when `rows` is empty. `.tradehistory-empty` CSS added to `bots.css`. Existing empty-state tests updated to assert on the message text. |
| ~~LT9~~ | ~~Remove dead code~~ | ~~1h~~ | ~~Info~~ | ✅ **Done** — `App.css`: removed `.App-logo`, `.App-header`, `.App-link`, `App-logo-spin` keyframe (kept `.page-loader`). `styles.css`: removed unused `.card` class. `package.json`: removed `@testing-library/user-event` (installed but never used). Lockfile updated. |
| ~~LT10~~ | ~~Enable Web Vitals reporting~~ | ~~30min~~ | ~~Info~~ | ✅ **Done** — `VITE_VITALS_URL` documented (commented out) in `.env.production` with clear instructions; `.env.production.example` updated to use correct `VITE_` prefixes throughout. Operators uncomment and set the URL to enable reporting. |
| ~~LT11~~ | ~~Remove `coingecko.com` from CSP `connect-src`~~ | ~~15min~~ | ~~Info~~ | ✅ **Done** — `https://api.coingecko.com` removed from `nginx.conf` CSP `connect-src`; comment updated to remove CoinGecko reference. |
| ~~LT12~~ | ~~Add JSDoc to `api.ts` exported functions~~ | ~~1–2h~~ | ~~Info~~ | ✅ **Done** — JSDoc added to all 10 exported functions: `setUnauthorizedHandler`, `fetchWsTicket`, `getBotIds`, `getOrders`, `getTrades`, `getDefaultParameters`, `getParameters`, `updateParameters`, `getDefaultIndicators`, `getIndicators`, `updateIndicators`. Each documents `@param`, `@returns`, and `@throws` where applicable. |
