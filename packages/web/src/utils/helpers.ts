...
export const fetchAllOrders = async (botIds: string[], clientId?: string): Promise<TradeRecord[]> => {
    const results = await Promise.all(botIds.map((id) => getOrders(id, clientId)));
    return results.filter((r): r is TradeRecord[] => r !== null).flat();
};

export const fetchAllTrades = async (botIds: string[], clientId?: string): Promise<TradeRecord[]> => {
